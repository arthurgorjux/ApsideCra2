...
abstract class Utilisateur implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\UtilisateurTableMap';
    ...
    /**
     * The value for the matricule field.
     *
     * @var        int
     */
    protected $matricule;
    ...
    /**
     * Initializes internal state of Base\Utilisateur object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [matricule] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [matricule] column.
     *
     * @param int $v new value
     * @return $this|\Utilisateur The current object (for fluent API support)
     */
    public function setMatricule($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if ($this->matricule !== $v) {
            $this->matricule = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_MATRICULE] = true;
        }
    ...
    public function setAdmin($v)
    {
        ...
        if ($this->admin !== $v) {
            $this->admin = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_ADMIN] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UtilisateurTableMap::translateFieldName('Matricule', TableMap::TYPE_PHPNAME, $indexType)];
            $this->matricule = (null !== $col) ? (int) $col : null;
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UtilisateurTableMap::translateFieldName('Admin', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 4; // 4 = UtilisateurTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Utilisateur'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_MATRICULE)) {
            $modifiedColumns[':p' . $index++]  = 'matricule';
        }
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_ADMIN)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'matricule':
                        $stmt->bindValue($identifier, $this->matricule, PDO::PARAM_INT);
                        break;
                    case 'nom':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_MATRICULE)) {
            $criteria->add(UtilisateurTableMap::COL_MATRICULE, $this->matricule);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_NOM)) {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_ADMIN)) {
            $criteria->add(UtilisateurTableMap::COL_ADMIN, $this->admin);
        }
    ...
    {
        $validPk = null !== $this->getMatricule();
    ...
    /**
     * Generic method to set the primary key (matricule column).
     *
    ...
    {
        $copyObj->setMatricule($this->getMatricule());
    ...
    public function setSuivis(Collection $suivis, ConnectionInterface $con = null)
    {
        ...
        foreach ($suivisToDelete as $suiviRemoved) {
            $suiviRemoved->setUtilisateur(null);
        }
